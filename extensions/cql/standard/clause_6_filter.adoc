...
<<filter-param,`filter`>> parameter is the <<basic-cql-text,text encoding of CQL>>.

The enumerated value `cql-json` is used to indicate that the value of the
<<filter-param,`filter`>> parameter is the <<basic-cql-JSON,JSON encoding of CQL>>.
...
A <<cql-core,Basic CQL core>> is defined along with a
number of <<cql-enhancements,enhancements>> that add additional, more advanced
capabilities to the language. Two encodings, a <<basic-cql-text,text encoding>> 
and a <<basic-cql-json,JSON>> encoding are also defined and
