...
This requirements class adds support for the operators LIKE, BETWEEN and IN as well as for pre-defined string functions UPPER() and LOWER().
...
include::recommendations/spatial-operators/PER_spatial-predicates.adoc[]
...
NOTE: The spatial predicates currently use a different pattern than the comparison and temporal predicates and are written as function calls.  This difference is inherited from previous versions of CQL2. This is an open issue: Whether this version of CQL2 used in the OGC API - Features - Part 3 standard should change the spatial predicates to be consistent with the other predicates or to keep the legacy because of existing CQL2 implementations.  Comments are welcome in the https://github.com/opengeospatial/ogcapi-features/issues/493[GitHub issue] on this topic.
