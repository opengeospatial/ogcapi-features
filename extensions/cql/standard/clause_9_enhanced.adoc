...
In each case, the clause also amends and extends CQL's <<cql-bnf,BNF rules>> and the text and
...
This clause specified requirements for support array expression is CQL.  Array
expressions can be tested in a predicate for equality, if one array is a subset of another, it one array is a superset of another or is two arrays overlap or share elements.
