...
Basic CQL only requires support for property-literal comparisons. That means that servers only has to support property references as expressions on the left hand side of an operator, and only literal values as expressions on the right hand side.
