...
As logical schemas, the schemas describe the data independent of a particular format and encoding to represent the data. 
...
In the OGC Web API Standards suite, geospatial data is shared in collections (path `/collections/{collectionId}`). The schema of items in a collection provides information as to how to interact with the collection. 
