...
** "pattern": All property values match a regular expression pattern; and
...
** "minimum", "maximum", "exclusiveMinimum", "exclusiveMaximum": The property values are from a range; and
...
* For textual values:
** "enum": The list of actual values, if the values are from a finite set; and
** "minLength", "maxLength": The actual range of the length of the text strings.
* For numeric values:
** "enum": The list of actual values, if the values are from a finite set (integer only); and
