...
=== Requirements and recommendations for schemas in OGC Web API Implementations

These requirements classes specify API building blocks related to schemas. 

As specified in OGC Web API Standards, geospatial data is shared in collections (path `/collections/{collectionId}`). The schema of items in a collection provides information about how to interact with the collection. 
