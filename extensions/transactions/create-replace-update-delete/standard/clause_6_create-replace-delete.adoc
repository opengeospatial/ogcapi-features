...
Note that the POST, PUT and/or DELETE methods will typically only be available for authorized users with sufficient permissions. The available methods of a user are available via the <<options,OPTIONS method>> for each resource.
