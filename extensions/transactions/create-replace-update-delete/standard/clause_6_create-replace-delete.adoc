...
replace and/or remove individual resources in a collection of resources.  Not all operations need to be implemented for all offered mutable resources.

NOTE: The term "collection" is used in the general sense of a set of resources of the same type, for example, the features in a feature collection (`{landingPageUri}/collections/{collectionId}/items`), the styles of a dataset (`{landingPageUri}/styles`), or the processes offered by a server (`{landingPageUri}/processes`). See also the note in the Scope clause.
...
The *Status Codes* listed in <<status_codes>> are of relevance to implementors of this Standard. Status codes 200, 201, 202, 204 and 404 are called out in this Standard's requirements and permissions. Therefore, support for some of these status codes is mandatory for all compliant implementations. The remainder of the status codes in <<status_codes>> are not mandatory but are important for the implementation of a well-functioning API. Support for these status codes is strongly encouraged for both client and server implementations.
...
A server may not execute a CREATE, REPLACE or DELETE operation immediately, for example, because the changes are reviewed before they are applied, or because the operation is added to a processing queue. In this case, the server responds with the HTTP status code `202` (see <<rfc9110,section 15.3.3 of RFC 9110>>).

include::recommendations/create-replace-delete/PER_async.adoc[]

This Standard does not specify how a client can determine the outcome of an operation that is processed asynchronously. The processing can succeed or fail, without further notification to the client. In particular, a client that has submitted a CREATE request does not know the URI of the resource that will be created.

NOTE: The intention is to add support for asynchronous processing in a future version of this Standard: A client would use the `respond-async` and `wait` preferences of the `Prefer` header (<<rfc7240,RFC 7240>>) to state whether it wants the operation to be processed asynchronously and how long it is willing to wait for a synchronous response. For an asynchronously processed operation, the server would publish a Job resource at `{landingPageUri}/jobs/{jobId}` that describes the status and outcome of the operation and reference the resource in the `Location` header of the `202` response, following the pattern of <<OAProc-1,OGC API - Processes - Part 1: Core>>.

[[cross_origin]]
==== Cross-origin requests

See <<OACommon-1,OGC API - Common - Part 1: Core>>, section link:https://www.opengis.net/spec/ogcapi-common-1/1.0#cross-origin-section[Support for cross-origin requests],
about the importance of supporting cross-origin requests, typically via link:https://en.wikipedia.org/wiki/Cross-origin_resource_sharing[Cross-origin resource sharing (CORS)].

==== Schemas

This Standard makes no assumptions about any schema constraints that a server may impose upon incoming resource data.
The intent is to support both "schema-less" servers (such as those back-ended by document data stores) as well as servers that have underlying schema constraints
(such as those back-ended by an RDBMS).

In the "schema-less" case, as long as the incoming resource meets the requirements of the declared media type (e.g., a valid GeoJSON feature) then the resource should be
created/updated with a 20x status response.

In the case where the server imposes schema constraints, the server should publish schemas to help clients create requests that are not rejected.
For example, in the case of features, schemas can be obtained from the `{landingPageUri}/collections/{collectionId}/schema` endpoint, as described in clause <<schemas,Feature schemas>>.
...
If the server does not execute the create request immediately, but accepts it for processing at a later time (see Permission <<per_create-replace-delete_async,/per/create-replace-delete/async>>), the response has the HTTP status code `202`. In this case, the resource creation can succeed or fail, without further notification to the client about the result or the resource identifier. See <<async_processing,Asynchronous processing>>.
...
The second example adds a new style to the styles of a dataset. The style is represented as a Mapbox style (see the draft <<OAStyles,OGC API - Styles>> standard).

[#create-style-example,reftext=`Create Style Example`]
.Create a New Style Example
include::../examples/json/ex09_create-style.adoc[]
...
The status code depends on the server. If the server has replaced the resource, the response is either `200` if the response includes additional content, or `204` if the response has no additional content. If the resource did not exist and the server has created it (see Permission <<per_create-replace-delete_put-create,/per/create-replace-delete/put-create>>), the response is `201`. 

If the server does not execute the request immediately, but accepts it for processing at a later time (see Permission <<per_create-replace-delete_async,/per/create-replace-delete/async>>), the response has the HTTP status code `202`. In this case, the processing can succeed or fail, without further notification to the client. See <<async_processing,Asynchronous processing>>.
...
include::recommendations/create-replace-delete/delete/REC_no-resource.adoc[]
...
The `Accept-Post` header informs clients which representations of a new resource the server can process. There is no such header for the REPLACE operation; clients can determine the supported media types from the API definition, if the server publishes one, or by submitting the request and evaluating a `415` response.

include::requirements/create-replace-delete/options/REQ_accept-post.adoc[]
...
Hints that are relevant to the operations specified in this Standard include:

- `allow` (the HTTP methods that can be used with the resource, that is, the same information as in the `Allow` header);
- `accept-post` and `accept-patch` (the media types that the resource accepts in the request body of a CREATE or UPDATE operation, that is, the same information as in the `Accept-Post` and `Accept-Patch` headers);
- `formats` (which, for a resource that allows PUT, includes the media types accepted in a REPLACE operation, for which no response header field is defined);
- `precondition-req` (whether the resource expects a precondition in state-changing requests, where the values `etag` and `last-modified` correspond to the <<rc_optimistic-locking-etags,Optimistic Locking using ETags>> and <<rc_optimistic-locking-timestamps,Optimistic Locking using Timestamps>> requirements classes);
- `accept-prefer` (the preferences specified in <<rfc7240,RFC 7240>> that the resource understands, for example `handling=strict` and `handling=lenient`, see the <<rc_handling,Handling Preference>> requirements class); and
- `auth-schemes` (the HTTP authentication schemes that the resource requires).
