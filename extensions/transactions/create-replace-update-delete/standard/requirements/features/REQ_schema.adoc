...
^|D |If the JSON Schema includes a statement `"additionalProperties": false`, the server SHALL reject mutation requests with a property that is not specified in the JSON Schema.
^|E |The server SHALL reject mutation requests that set a property which is specified in the JSON Schema with a statement `"readOnly": true`.
|===
