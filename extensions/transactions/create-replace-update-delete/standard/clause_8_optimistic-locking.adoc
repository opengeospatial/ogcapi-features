...
include::requirements/optimistic-locking-timestamps/REQ_ifunmodifiedsince-delete-op.adoc[]

include::requirements/optimistic-locking-timestamps/REQ_ifunmodifiedsince-delete-eval.adoc[]
...
Client A successfully replaces the resource while Client B's request fails.
Client B, then proceeded to re-fetch the resource and retry the request.
...
include::requirements/optimistic-locking-etags/REQ_ifmatch-delete-op.adoc[]

include::requirements/optimistic-locking-etags/REQ_ifmatch-delete-eval.adoc[]
...
The following example, taken from the <<optimistic-locking-overview>>, optimistically locks the resources being operated on using the https://www.rfc-editor.org/rfc/rfc9110#field.if-match[If-Match] header.  Client A successfully replaces the resource while Client B's request fails.  Client B, then proceeded to re-fetch the resource and retry the request.
