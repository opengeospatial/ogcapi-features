...
A server that implements this conformance class generates a https://www.rfc-editor.org/rfc/rfc7232#section-2.2[Last-Modified] header when the resource if fetched or modified representing the last time the resource was changed and recognizes the https://www.rfc-editor.org/rfc/rfc7232#section-3.4[If-Unmodified-Since] conditional HTTP header in order to implement optimistic locking of resources to support concurrency.

[[last-modified]]
==== Last-modified 

As defined in https://www.rfc-editor.org/rfc/rfc7232.html[RFC 7232], for the purposes of this conformance class, the value of the https://www.rfc-editor.org/rfc/rfc7232#section-2.2[Last-Modified] header is a timestamp indicating the date and time at which the origin server believes that the selected representation was last modified, as determined at the conclusion of handling the request.
...
Optimistic resource locking is implemented using the https://www.rfc-editor.org/rfc/rfc7232#section-3.4[If-Unmodified-Since] HTTP header.  The value of the header is evaluated according to rules defined in https://www.rfc-editor.org/rfc/rfc7232.html[RFC 7232] and the result of that evaluation is used to determine if the operation shall be performed or not.

If the https://www.rfc-editor.org/rfc/rfc7232#section-3.4[If-Unmodified-Since] HTTP header evaluation results in a value of `true` then the operation can proceed.

If the https://www.rfc-editor.org/rfc/rfc7232.html#section-3.4[If-Unmodified-Since] HTTP header evaluation, however, evaluates to `false` then the server generates a response according to the rules defined in https://www.rfc-editor.org/rfc/rfc7232.html#section-3.4[RFC 7232].  Normally, in this case, the response is a HTTP status code of 412, precondition failed.  However, if the requested change is already reflected in the current state of the resource then the server can respond with a 2xx, successful HTTP status code.
...
the https://www.rfc-editor.org/rfc/rfc7232.html#section-3.4[If-Unmodified-Since] header.
...
[D]|<--------------------------------------------------|                                                   |
   ...
   |                                                   |                                                   |
   | PUT /col...ns/oakland_buildings/items/OB.2        |                                                   |
   | If-Unmodified-Since: Sat,13 Mar 2021 11:28:17 GMT |                                                   |
[E]|-------------------------------------------------->|                                                   |
   |                                                   |                                                   |
   | HTTP/1.1 204 No Content                           |                                                   |
   | Last-Modified: Sun,14 Mar 2021 13:14:15 GMT       |                                                   |
[F]|<--------------------------------------------------|                                                   |
   ...
   |                                                   |                                                   |
   |                                                   | PUT /col...ns/oakland_buildings/items/OB.2        |
   |                                                   | If-Unmodified-Since: Sun,14 Mar 2021 13:14:15 GMT |
   |                                                   |<--------------------------------------------------|[K]
   |                                                   |                                                   |
   |                                                   | HTTP/1.1 204 No Content                           |
...
A server that implements this conformance class generates an https://www.rfc-editor.org/rfc/rfc7232.html#section-2.3[entity tag] representing the state of the resource when the resource is fetched or modified and recognizes the https://www.rfc-editor.org/rfc/rfc7232.html#section-3.1[If-Match] conditional HTTP header in order to implement optimistic locking of resources to support concurrency.
