...
| `create-building-with-id.json` | A valid new building feature whose representation contains an `id` member, for the assertions of the CREATE tests about the identifier of the new resource: the server may use the identifier submitted in the request body or assign its own (Permission `/per/create-replace-delete/rid`), and the `Location` header states the identifier that was assigned. Otherwise `create-building.json`, with another `name` so that the two features can be told apart; the identifier `B.n1` is the `gml:id` of `create-building.gml`, where an identifier cannot be omitted. |
