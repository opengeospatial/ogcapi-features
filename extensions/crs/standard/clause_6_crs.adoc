...
the approach specified in this draft standard should also be applicable to
...
The list of supported CRS identifiers in the collection object may include
...
the <<crs-discovery,collection object>>, a global list of supported CRS
identifiers may be provided as part of the collections object.
...
must be explicitly referenced in the collection object using a
...
CRSs specified in the global list plus five other CRSs. The `tor_buildings`
collection is offered in the CRSs  specified in the global list.  The
`dc_buildings` collection, not having a `crs` property, is only offered in
...
such a capability is considered only for a future version of this standard.
...
NOTE: HTML only supports WGS84 based on the schema.org dependency. The OGC is not sure if this is an issue but schema.org annotations seem to require WGS84 (lat,lon) yet <<OAFeat-1,OGC API - Features - Part 1: Core>> requires lon,lat.
...
A related general question, is whether that capability to state the axis order
