...
NOTE: To support the future use of datasets with items that are not feature,
...
at least a link to the items in each `Collection` (path `/collections/{collectionId}/items`,
link relation `items`) as well as key metadata about each collection
(`Collection metadata`). The metadata includes:
...
The `Collection metadata` for each  `Collection` is available at path
...
as items in the collection. This document only discusses the behavior of feature
collections.
...
Accessing a `Collection` using HTTP GET returns a document consisting of
