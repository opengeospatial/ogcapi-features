...
provide guidance on which (common) error situations should return which
HTTP status code per resource or should we simply require that http status codes
are used correctly?
...
This specification does not specify the parameter `bbox-crs`. This parameter
will be specified in an additional part of the WFS 3.0 series.
