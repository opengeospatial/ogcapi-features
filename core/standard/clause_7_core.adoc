...
instead, e.g. `/buildings`, `/buildings.json` and `/buildings.html`.
This is clearer and avoids a conflict with content negotiation.
...
`json` / `html` are reserved values and SHALL return the first
JSON / HTML media type listed in the API definition. If no such
media type is included in the API definition, the service MAY determine
the format of the response or respond with HTTP status code `406`.
