...
an OpenAPI definition that returns a OpenAPI document. Maybe we should simply
require that the document is available at /api, drop the example and exclude
the /api operation from all other examples, too?
...
SHALL be based upon the schema https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/schemas/exception.yaml.
|===
...
https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/parameters/f.yaml.
...
HTTP status code "200" based upon the schema https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/schemas/content.yaml.
|===
...
NOTE: TBD +
Maybe we need something more specific than just `describedBy` so that the different cases can be distinguished by a client?
...
NOTE: TBD +
This would duplicate the links in the HTTP header, but maybe this is ok as these are the most important links?
...
https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/parameters/startIndex.yaml.
...
A template for the definition of the parameter in YAML is available at
https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/parameters/count.yaml.
...
A template for the definition of the parameter in YAML is available at
https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/parameters/bbox.yaml.
...
A template for the definition of the parameter in YAML is available at
https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/parameters/resultType.yaml.
...
https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/schemas/timeStamp.yaml.
|===
...
TBD +
Maybe it would be better to just use HTTP headers for this? This would have
the great advantage that it will be possible to just reuse existing containers.
For example, to conform to GML-SF0, the feature collection cannot have such
attributes. The only disadvantage that I see is that if just the payload is saved
the information is "lost", but how much of a problem is that really?
=====================================================================
