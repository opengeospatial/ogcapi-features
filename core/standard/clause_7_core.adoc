...
The `Page` may include metadata about the number of selected and returned
...
NOTE: In the description the term "collection" was used where "feature collection"
would have been more precise as this document only discusses datasets of features.
To support also future use of the `Collections` and `Collection` resources for
other collections of other resource types "feature" has not been added in either
the names of the resource types or their paths.
...
features in the dataset via additional, deeper collection hierarchies.
...
like the `startIndex` parameter that was used in <<WFS20,WFS 2.0>> (and
which may be added again in additional parts of the OGC API Features series.
