...
Change wording, if the API definition may be located at any URI and/or if
`service-desc` and `service-doc` are used as link relations.
...
----
{
  ...
  "links": [
    ...
    { "href": "http://data.example.org/api",
      "rel": "service", "type": "application/vnd.oai.openapi+json;version=3.0", "title": "the API definition" },
...
Two example OpenAPI documents are included in <<oas30_example,Annex B>>. The
documents differ in their representation of the feature collections. The
...
.link:https://raw.githubusercontent.com/opengeospatial/WFS_FES/master/core/openapi/schemas/collections.yaml[Schema for the metadata about feature collections]
[source,YAML]
----
type: object
required:
