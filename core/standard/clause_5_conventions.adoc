...
This version is a draft. It is not complete and there are open issues that
are under discussion. These cases are shown as "CAUTION" annotations with
a link to the issue on GitHub and a brief summary of the issue.

The current expectation is to have a stable version of the Core specification
in 2019. Criteria to move the specification to the next stage in the
process are:
