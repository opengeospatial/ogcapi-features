...
used by a OGC API Features implementation. These encodings are commonly used encodings for spatial data on
the web:
...
|Conformance classes |`/conformance` |`core:ConformsTo`
|Feature collections metadata |`/collections` |`core:Collections`
|Feature collection metadata |`/collections/{collectionId}` |`core:Collections`, with just one entry for the collection `collectionId`
|Feature collection |`/collections/{collectionId}/items` |`core:FeatureCollection`
|Feature |`/collections/{collectionId}/items/{featureId}` |substitutable for `gml:AbstractFeature`
!===

The namespace prefixes used above and in the OGC API Features Core XML Schema are:

* `core`: `http://www.opengis.net/ogcapi-features-core/1.0`
...
NOTE: The `core:FeatureCollection` element deliberately goes beyond the permitted content
