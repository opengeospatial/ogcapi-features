...
In version 1.0 of this standard, the XML encoding also covered the landing page, the conformance declaration, the collections and the collection resource, using the root elements `core:LandingPage`, `core:ConformsTo` and `core:Collections`. The XML encoding of these four resources is deprecated in this version and this document no longer states requirements for it. The elements remain in the OGC API Features Core XML Schema and are marked as deprecated there.

The reasons are that there has been no noticeable uptake of the XML encoding of these resources in clients, that extending their content model in other parts of the OGC API Features series adds complexity to the standards and to implementations that is not justified by that uptake, and that the OGC API - Common standards specify only JSON and HTML for the same resources.

NOTE: This does not affect the support for GML as a feature encoding. The Features and Feature resources are not affected.
