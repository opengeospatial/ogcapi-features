...
The server SHALL support the HTTP GET operation at the path /collections/{collectionId}. collectionId is the `id` property in the feature collections metadata. +
Tests: A.4.4.7 +
...
For every feature collection identified in the metadata about the feature collection (path /), the server SHALL support the HTTP GET operation at the path /collections/{collectionId}/items where {collectionId} is the `id` property in the feature collections metadata. +
Tests: A.4.4.9 +
...
For every feature in a feature collection (path /collections/{collectionId}/items), the service SHALL support the HTTP GET operation at the path /collections/{collectionId}/items/{featureId} where {collectionId} is the `id` property in the feature collection metadata and {featureId} is a local identifier of the feature. +
Tests: A.4.4.14 +
