...
== A.3: Abstract Tests
WFS 3.0 is a highly modular standard.  One result of this is that almost everything is optional.
. Are the advertised capabilities compliant with the WFS 3.0 standard?
. Does the service implement those capabilities as advertised?
 +
=== A.3.1: Core Test:
...
Test Method: +
. Retrieve the API Description document from {Root_URL}/api +
. Process the API Description document based on the API standard used +
...
Test Method: +
. Validate that the API Description document complies with the OpenAPI standard.
. Retrieve each of the security controls from {API_Description}/security
. IF there are security controls which were not used to retrieve the API Description, retrieve the API Description using the most restrictive controls specified.  All further access to the service will be preformed using these controls.
. Count the number of entries in {API_Description}/servers +
    (ISSUE: What do we do if there is more than one?) +
. Retrieve the first Server Object from {API_Description}/servers +
     a) Retrieve the /{Server_Object}/url value.  +
     b) Set {Root_URL} to this value +
     (ISSUE: what do we do with multiple server URLs?) +
. Retrieve all Components Objects from {API_Description}/components and save for latter use.
. Retrieve all Tag Objects from {API_Description}/tags and save for latter use.
. Process each element in {API_Description}/paths using test A.3.3.
----
=== A.3.3: OpenAPI Paths
