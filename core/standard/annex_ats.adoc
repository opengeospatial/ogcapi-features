...
NOTE: The Abstract Test Suite is still work-in-progress.

=== Introduction

OGC API Features is not a Web Service in the traditional sense. Rather, it defines the behavior and content of a set of Resources exposed through a Web Application Programing Interface (Web API). Compliance testing for OGC API Features and similar standards must answer three questions:

.  Are the capabilities advertised through the API definition compliant with the standard?
