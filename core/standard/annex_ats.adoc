...
Conventions:
URL templates are used throughout these test suites.  Path templating refers to the usage of curly braces ({}) to mark a section of a URL path as replaceable using path parameters.  The terms used to describe portions of these templates are based on the URL syntax described in RFC 3986.
. scheme: http | https
...
* Test Method:
{validate /compliance first to get the compliance information}
  1) FOR EACH entry in {API_Description}/paths
  ...
  2) Verify that all paths required by WFS 3.0 were included
...
* Test Method:
  ...
  5) Validate the operations described by this Path Item:
      ...
      "/api
          ...
          Perform Test A.*.* using {Operation_Object}
      "/config"
...
Paths metadata:
1) Identifies the HTTP operations (get, put, etc) that can be executed on this path.  EAch opearation identified has associated metadata in an Operations Object.
2) Servers - a list of Server Objects which identify alternate servers for this operation.  ISSUE: If alternate servers are identified, do they need to be tested for compliance as well?
3) Parameters which are applicable for all operations in this path.  These may be overridden at the operations level.
...
Operations Metadata:
{Operation_Object}/operationId - not required but may be helpful.
{Operation_Object}/parameters - same as above
{Operation_Object}/requestBody
{Operation_Object}/responses
{Operation_Object}/callbacks - not currently used by the WFS 3.0 standard
{Operation_Object}/security
{Operation_Object}/servers



=== A.3: OpenAPI 3.0 Test Suites*
