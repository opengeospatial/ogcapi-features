...
Features and Feature Collections use the same logical data model regardless of whether they are hosted on a WFS 2.0 or WFS 3.0. That includes support for the gml:id attribute or an enquivalent. Implementors should pay particular attention to this property when designing their systems.  

Geospatial data tends to wander. It is common practice to widely distribute this data, sending copies wherever they are most convienent for the users. In addition, Features and Feature Collections are often encorporated into derivative products. The same resource can appear in many different data sets. Finally, there are often legal and historical requirements to identify a specific geospatial resource decades or even centuries after it was first published. The identifier property allows us to track geospatial Features as they migrate through space and time.
...
WFS 2.0 uses a query-response model to access its' data. The hosted Features and Feature Collections are not individually addressable. WFS 3.0 provides a path heirarchy to access its' data. Individual Features and Feature Collections are individually accessible as long as you know the path and the id of the resource.
...
This approach can work if the data holdings are stable and static. In a more dynamic environment, the contents of the result set may change between the initial and subsequent requests. The implementor has a choice, either return invalid data or re-execute the initial request. 
...
The WFS 3.0 Standard takes no position on this issue. It is a decision of the implementor based on the nature of their data and requirements of prospective users.
...
*Recomended Practice:* OpenAPI allows you to assign a name to each operation. By assiging the appropriate WFS 2.0 operation name to each WFS 3.0 operation, the relationship between the two standards will be clearer to client developers.
