...
NOTE: Although the `CASEI` function is used in CQL2 to trigger case insensitive matching, how this function is mapped to an underlying query engine is outside the scope of this specification.  In some cases this mapping may be to a function such as `UPPER()` which is commonly available in many RDBMs.  In other cases, the mapping may be to include a clause in the predicate that signals the use of a case-insensitive collation.
