...
Implementations of the `CASEI` operator can be complex, but in many cases the underlying datastore will provide a capability that the operator can be mapped to.

include::requirements/case-insensitive-comparison/REQ_casei-builtin-function.adoc[]
...
----
{
  "op": "in",
  "args": [
    { "function": { "name": "casei", "args": [{ "property": "road_class" }] } },
    [
...
Similar to the case-insensitive comparison, this capability is supported via an operator `ACCENTI`. 
...
include::requirements/accent-insensitive-comparison/REQ_accenti-builtin-function.adoc[]
...
----
{
  "op": "=",
  "args": [
    { "function": { "name": "accenti", "args": [{ "property": "etat_vol" }] } },
...
Like `CASEI`, the `ACCENTI` operator returns a string typed representation of the input expression that is guaranteed to be equal to any other accent insensitive representation of that string.
