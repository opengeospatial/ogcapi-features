...
In the Text representation, the type is encoded as a `BBOX()` operator with four or six numerical arguments, depending on whether the coordinates include a vertical axis (height or depth):
...
In this conformance class, the only required spatial operator is _intersects_ and the only required spatial data types are point and BBox. Additional spatial operators and spatial data types are specified in the <<rc_spatial-operators,Spatial Operators>> requirements class.
...
include::recommendations/basic-spatial-operators/PER_spatial-data-types.adoc[]
...
* a set of Dimensionally Extended Nine-intersection Model (DE-9IM) relation operators that may be used to add spatial predicates to a CQL2 filter expression.

[[additional-spatial-data-types]]
==== Additional spatial data types and literal values

include::requirements/spatial-operators/REQ_spatial-data-types.adoc[]
