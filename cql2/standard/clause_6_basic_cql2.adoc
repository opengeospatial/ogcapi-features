...
A collection item that satisfies ALL the requirements of a CQL2 filter expression according to the above true tables evaluates to a Boolean value of `TRUE`; otherwise the CQL2 filter expression evaluates to `FALSE` or `NULL`.
